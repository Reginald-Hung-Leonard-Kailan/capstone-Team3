    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .and()
                    ...
                    .requestMatchers("/", "/register", "/login", "/css/**", "/js/**", "/img/**", "/stats") // anyone can see home, the ads pages, and sign up
                    .permitAll()
                .and()
                    .authorizeHttpRequests()
                    .requestMatchers(
                            "/home", // only authenticated users can create ads
                            "/client", "/stats", "/edit/{id}", "/client-edit/{id}", "/users/search", "/add-injury/{id}", "/add-injury/**", "/injury/edit/{id}", "/injury/delete/{id}", "/client-false/{id}", "/api/user/**" , "/api/user" // only authenticated users can edit ads
                    )
                    .authenticated()
        ;
        return http.build();
